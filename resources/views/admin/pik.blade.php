...
<body class="">
      ...
      <nav class="navbar navbar-expand-lg navbar-absolute fixed-top navbar-transparent">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            ...
            <a class="navbar-brand" href="#">Persatuan Ibu dan Karyawati - PIK</a>
          </div>
  ...
  <canvas id="bigDashboardChart"></canvas>
      ...
      <div class="row">
          <div class="col-md-12">
                ...
                <div class="table-responsive">
                  <table class="table">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td>
                         <p> {{$fotopik->filename}}</p>
                        </td>
                        <td>
                            <img src="{{ asset ('upload/Pik') }}/{{$fotopik->filename}}" style="height: 120px; width: 170px;">
                        </td>
                        <td>
                              <form action="{{action('PikController@destroy', $fotopik['id'])}}" method="post">
                              {{ csrf_field() }}
                  ...
                  <form action="{{ route('pik')}} " method="post" class="form-horizontal" enctype="multipart/form-data">
                  {{ csrf_field() }}
            ...
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">Anggota PIK</h4>
              </div>
              <div class="card-body">
                <div class="table-responsive">
                  <table class="table">
                    <strong>Create Data Anggota</strong><br><br>
                    <form action="{{ route('karyawanpik')}} " method="post" class="form-horizontal" enctype="multipart/form-data">
                      {{ csrf_field() }}
                    ...
                    <tbody>
                      @foreach( $anggotapik as $dataanggota)
                      <tr>
                        <td>
                        ...
                        <td>
                          <form action="{{action('AnggotapikController@destroy', $dataanggota['id'])}}" method="post">
                        {{ csrf_field() }}
                              <input name="_method" type="hidden" value="DELETE">
                              <button class="btn btn-danger" type="submit">Delete</button>
                           </form>
                           {{ csrf_field() }}
                              <button class="btn btn-success" type="submit" data-toggle="modal" data-target="#pikmodal" data-id="{{$dataanggota->id}}"
                              ...
                              @endforeach
                              <div class="modal fade" id="pikmodal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog" role="document">
                          ...
                          <div class="modal-content form-elegant">
                            ...
                            <!--Body-->
                            <form role="form" action="{{ route('updatepik')}}" enctype="multipart/form-data" method="post">
                                  {{csrf_field()}}
  ...
  <script>
    ...
    <script>
      //modal
      $('#pikmodal').on('show.bs.modal', function (event){
